...
def run_manual_script_with_json_to_zip(
    ...
    """
    Runs the manual assembly protocol script with an assembly-plan JSON file,
    captures stdout/stderr, and writes a ZIP file next to the original script.
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        (tmpdir / "manual_protocol_stdout.txt").write_text(
            proc.stdout or "", encoding="utf-8", errors="replace"
        )
        (tmpdir / "manual_protocol_stderr.txt").write_text(
            proc.stderr or "", encoding="utf-8", errors="replace"
        )
        (tmpdir / "manual_protocol_returncode.txt").write_text(
        ...
        if proc.returncode != 0:
            ...
            raise RuntimeError(
                f"Manual protocol generation failed with exit code {proc.returncode}."
