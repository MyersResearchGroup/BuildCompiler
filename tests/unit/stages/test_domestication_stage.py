...
def _inventory(with_backbone=True, with_enzyme=True, with_ligase=True) -> Inventory:
    ...
    if with_enzyme:
        reagents.append(
            IndexedReagent("e1", name="BsaI", reagent_type="restriction_enzyme")
        )
    if with_ligase:
        reagents.append(
            IndexedReagent("l1", name="T4_DNA_ligase", reagent_type="ligase")
        )
...
def test_blocked_when_backbone_missing() -> None:
    ...
    result = stage.run(
        _request(), source_document=_source_doc(), target_document=sbol2.Document()
    )
...
def test_blocked_when_reagents_missing() -> None:
    stage = DomesticationStage(
        inventory=_inventory(with_enzyme=False, with_ligase=False)
    )
    result = stage.run(
        _request(), source_document=_source_doc(), target_document=sbol2.Document()
    )
